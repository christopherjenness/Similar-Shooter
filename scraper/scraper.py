...
def get_player_urls(year):
    """
    Get BasketballReference URLs for all players in a given year

    Args:
        year (int): year to get active players's URLS

    Returns: list
        list of BR URLs
    """
    url = ("http://www.basketball-reference.com/"
           "leagues/NBA_{year}_totals.html").format(year=year)
...
def make_shooting_url(baseurl):
    ...
    return ("http://www.basketball-reference.com"
            "{baseurl}/shooting/2017/").format(baseurl=baseurl)
...
def make_league_dfs(cache=True):
    ...
    cols = ['At Rim', '3 to <10 ft', '10 to <16 ft',
            '16 ft to <3-pt', '3-pt']
    FGA.columns = cols
    FGP.columns = cols
    if cache:
