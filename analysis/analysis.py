...
def load_data(min_shots=250):
    ...
    Args:
        min_shots (Int): Minimum number of shots required to retain player in data

    Returns: (df.DataFrame, df.DataFrame)
        tuple of pd.DataFrames of FGA and FG% for various court locations for each player
    """
...
def get_similar_shooters(target_player, FGA_df, FGP_df):
    """
    Finds similar shot chart for target_player via KL-divergence of FGA distributions

    Args:
        target_player (str): Player to find similar shot charts for
        FGA_df (pd.DataFrame): DataFrame of FGA distributions by court location for each player
        FGP_df (pd.DataFrame): DataFrame of FG% distributions by court location for each player
